...
from database_setup import Base, Department, CategoryItem, User
...
APPLICATION_NAME = "Sporting Goods Catalog Application"
...
# JSON APIs to view department Information
@app.route('/department/<int:department_id>/category/JSON')
def departmentCategoryJSON(department_id):
...
@app.route('/department/<int:department_id>/category/<int:category_id>/JSON')
def categoryItemJSON(department_id, category_id):
...
def showDepartments():
    ...
    if 'username' not in login_session:
        return render_template('publicdepartments.html', departments=departments)
    else:
...
def editDepartment(department_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('editDepartment.html', department=editedDepartment)


# Delete a department
@app.route('/department/<int:department_id>/delete/', methods=['GET', 'POST'])
def deleteDepartment(department_id):
    departmentToDelete = session.query(
        Department).filter_by(id=department_id).one()
    if 'username' not in login_session:
        return redirect('/login')
    ...
    if request.method == 'POST':
        ...
        flash('%s Successfully Deleted' % departmentToDelete.name)
        session.commit()
        return redirect(url_for('showDepartments', department_id=department_id))
    else:
        return render_template('deleteDepartment.html', department=departmentToDelete)

# Show a department category


@app.route('/department/<int:department_id>/')
@app.route('/department/<int:department_id>/category/')
def showCategory(department_id):
    department = session.query(Department).filter_by(id=department_id).one()
...
@app.route('/department/<int:department_id>/category/new/', methods=['GET', 'POST'])
def newCategoryItem(department_id):
    if 'username' not in login_session:
        return redirect('/login')
    department = session.query(Department).filter_by(id=department_id).one()
    if login_session['user_id'] != department.user_id:
        return "<script>function myFunction()" \
        "{alert('You are not authorized to add category items " \
        "to this department. Please create your own department " \
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('showCategory', department_id=department_id))
    else:
        return render_template('newCategoryItem.html', department_id=department_id)

# Edit a category item


@app.route('/department/<int:department_id>/category/<int:category_id>/edit', methods=['GET', 'POST'])
def editCategoryItem(department_id, category_id):
    if 'username' not in login_session:
        return redirect('/login')
    ...
    if login_session['user_id'] != department.user_id:
        ...
        "items to this department. Please create your own " \
        "department in order to edit items.');}" \
        "</script><body onload='myFunction()'>"
    if request.method == 'POST':
        if request.form['name']:
        ...
        return render_template('editCategoryItem.html', department_id=department_id,
        category_id=category_id,
        item=editedItem)


# Delete a category item
@app.route('/department/<int:department_id>/category/<int:category_id>/delete', methods=['GET', 'POST'])
def deleteCategoryItem(department_id, category_id):
    if 'username' not in login_session:
        return redirect('/login')
    department = session.query(Department).filter_by(id=department_id).one()
    ...
    if login_session['user_id'] != department.user_id:
        ...
        " items to this department. Please create your own " \
        "department in order to delete items.');}" \
        "</script><body onload='myFunction()'>"
    if request.method == 'POST':
